document.addEventListener("DOMContentLoaded", function () {
  const typingPhrases = ["Backend-Focused Fullstack Developer"];
  const subText =
    "Focused on backend development while growing as a fullstack developer through real-world web projects.";
  let phraseIndex = 0;
  let charIndex = 0;
  ...
  function animateSkills() {
    ...
    skillCards.forEach((card, index) => {
      const bar = card.querySelector(
        ".bg-blue-500, .bg-blue-400, .bg-yellow-400, .bg-cyan-500, .bg-purple-500, .bg-pink-500, .bg-green-500, .bg-blue-600, .bg-emerald-500, .bg-blue-700, .bg-gray-800, .bg-red-500",
      );
    ...
    for (let i = 0; i < 150; i++) {
      ...
      const animation = confetti.animate(
        [
          ...
          {
            opacity: 1,
            transform: `translateY(${window.innerHeight + 150}px) rotate(${Math.random() * 1080}deg)`,
          },
        ],
        {
